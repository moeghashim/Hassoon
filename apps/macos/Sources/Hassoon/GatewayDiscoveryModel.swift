...
final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "com.moeghashim.hassoon", category: "gateway-discovery")
    ...
    func start() {
        ...
        for domain in HassoonBonjour.bridgeServiceDomains {
            ...
            browser.start(queue: DispatchQueue(label: "com.moeghashim.hassoon.macos.gateway-discovery.\(domain)"))
        }
    }
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (Hassoon)", with: "")
