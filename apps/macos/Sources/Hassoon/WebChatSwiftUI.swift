...
import HassoonProtocol
import Foundation
import OSLog
import QuartzCore
import SwiftUI

private let webChatSwiftLogger = Logger(subsystem: "com.moeghashim.hassoon", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: HassoonChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> HassoonChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(HassoonChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> HassoonChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                HassoonGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<HassoonChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any HassoonChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = HassoonChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: HassoonChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<HassoonChatView>) -> NSViewController
    {
