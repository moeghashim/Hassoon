...
struct ConfigSettings: View {
    ...
    private static let browserAttachOnlyHelp =
        "When enabled, the browser server will only connect if the hassoon browser is already running."
    private static let browserProfileNote =
        "Hassoon uses a separate Chrome profile and ports (default 18791/18792) "
    ...
    // hassoon browser settings (stored in ~/.hassoon/hassoon.json under "browser")
    ...
    // Talk mode settings (stored in ~/.hassoon/hassoon.json under "talk")
    ...
    private var header: some View {
        Text("Hassoon CLI config")
            .font(.title3.weight(.semibold))
        Text(self.isNixMode
            ? "This tab is read-only in Nix mode. Edit config via Nix and rebuild."
            : "Edit ~/.hassoon/hassoon.json (agent / session / routing / messages).")
            .font(.callout)
    ...
    private var anthropicAuthHelpText: String {
        "Determined from Hassoon OAuth token file (~/.hassoon/credentials/oauth.json) " +
