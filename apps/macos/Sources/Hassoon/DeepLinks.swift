...
private let deepLinkLogger = Logger(subsystem: "com.moeghashim.hassoon", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "Hassoon is paused", message: "Unpause Hassoon to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Hassoon agent?", message: body) else { return }
        }
