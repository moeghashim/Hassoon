...
private let gatewayConnectionLogger = Logger(subsystem: "com.moeghashim.hassoon", category: "gateway.connection")
...
extension GatewayConnection {
    ...
    func healthOK(timeoutMs: Int = 8000) async throws -> Bool {
        ...
        return (try? self.decoder.decode(HassoonGatewayHealthOK.self, from: data))?.ok ?? true
    }
    ...
    func chatHistory(sessionKey: String) async throws -> HassoonChatHistoryPayload {
    ...
    func chatSend(
        ...
        attachments: [HassoonChatAttachmentPayload],
        timeoutMs: Int = 30000) async throws -> HassoonChatSendResponse
    {
        var params: [String: AnyCodable] = [
