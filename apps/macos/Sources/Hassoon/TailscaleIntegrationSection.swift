...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.hassoon/hassoon.json. Prefer HASSOON_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.hassoon/hassoon.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.hassoon/hassoon.json. Restart the gateway to apply."
        }
