...
actor BridgeServer {
    ...
    private let logger = Logger(subsystem: "com.moeghashim.hassoon", category: "bridge")
    ...
    func start() async {
        ...
        do {
            ...
            listener.start(queue: DispatchQueue(label: "com.moeghashim.hassoon.bridge"))
...
enum BridgePairingApprover {
    static func approve(request: BridgePairRequest, isRepair: Bool) async -> Bool {
        await withCheckedContinuation { cont in
            ...
            let alert = NSAlert()
            alert.messageText = isRepair ? "Re-pair Hassoon Node?" : "Pair Hassoon Node?"
