...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Hassoon token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: HassoonOAuthStore.AnthropicOAuthStatus = HassoonOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "com.moeghashim.hassoon", category: "anthropic-oauth")
...
enum HassoonOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let hassoonOAuthDirEnv = "HASSOON_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Hassoon OAuth token file not found"
            case .unreadableFile: "Hassoon OAuth token file not readable"
            case .invalidJSON: "Hassoon OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Hassoon OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Hassoon OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.hassoonOAuthDirEnv]?
