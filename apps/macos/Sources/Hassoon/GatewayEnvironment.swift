...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "com.moeghashim.hassoon", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func bundledGatewayExecutable() -> String? {
        guard let res = Bundle.main.resourceURL else { return nil }
        let path = res.appendingPathComponent("Relay/hassoon").path
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.hassoonExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expected?.description,
                    message: "hassoon CLI not found in PATH; install the global package.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["HASSOON_GATEWAY_BIND"] {
    ...
    static func installGlobal(version: Semver?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "hassoon@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "hassoon@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "hassoon@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "hassoon@\(target)"])
            }

        statusHandler("Installing hassoon@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed hassoon@\(target)")
        } else {
