...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: HassoonOAuthStore.AnthropicOAuthStatus = HassoonOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager.default.fileExists(atPath: HassoonOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = HassoonOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = HassoonOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try HassoonOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Hassoon can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: HassoonOAuthStore.AnthropicOAuthStatus,
