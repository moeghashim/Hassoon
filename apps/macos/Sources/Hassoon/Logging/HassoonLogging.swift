...
enum HassoonLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = HassoonOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = HassoonFileLogHandler(label: label)
    ...
    static func parseLabel(_ label: String) -> (String, String) {
        guard let range = label.range(of: Self.labelSeparator) else {
            return ("com.moeghashim.hassoon", label)
        }
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        HassoonLogging.bootstrapIfNeeded()
        let label = HassoonLogging.makeLabel(subsystem: subsystem, category: category)
...
struct HassoonOSLogHandler: LogHandler {
...
struct HassoonFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = HassoonLogging.parseLabel(self.label)
