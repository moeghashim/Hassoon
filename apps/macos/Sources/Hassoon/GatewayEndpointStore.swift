...
actor GatewayEndpointStore {
    ...
    struct Deps: Sendable {
        ...
        static let live = Deps(
            ...
            token: { ProcessInfo.processInfo.environment["HASSOON_GATEWAY_TOKEN"] },
            password: {
    ...
    private static func resolveGatewayPassword(
        ...
        let raw = env["HASSOON_GATEWAY_PASSWORD"] ?? ""
    ...
    private let logger = Logger(subsystem: "com.moeghashim.hassoon", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = UserDefaults.standard.bool(forKey: "hassoon.onboardingSeen")
