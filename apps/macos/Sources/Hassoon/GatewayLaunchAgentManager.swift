...
enum GatewayLaunchAgentManager {
    private static let logger = Logger(subsystem: "com.moeghashim.hassoon", category: "gateway.launchd")
    ...
    private static func gatewayExecutablePath(bundlePath: String) -> String {
        "\(bundlePath)/Contents/Resources/Relay/hassoon"
    }
    ...
    private static func writePlist(bundlePath: String, port: Int) {
        ...
        var envEntries = """
            ...
            <key>HASSOON_IMAGE_BACKEND</key>
            <string>sips</string>
        """
        if let token {
            let escapedToken = self.escapePlistValue(token)
            envEntries += """
                <key>HASSOON_GATEWAY_TOKEN</key>
        ...
        if let password {
            ...
            envEntries += """
                <key>HASSOON_GATEWAY_PASSWORD</key>
    ...
    private static func preferredGatewayToken() -> String? {
        let raw = ProcessInfo.processInfo.environment["HASSOON_GATEWAY_TOKEN"] ?? ""
