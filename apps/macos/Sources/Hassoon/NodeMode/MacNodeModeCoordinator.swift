...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "com.moeghashim.hassoon", category: "mac-node")
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [HassoonCapability.canvas.rawValue, HassoonCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(HassoonCapability.camera.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            HassoonSystemCommand.run.rawValue,
            HassoonSystemCommand.notify.rawValue,
        ]
    ...
    static func loopbackBridgePort() -> UInt16? {
        if let raw = ProcessInfo.processInfo.environment["HASSOON_BRIDGE_PORT"],
    ...
    static func probeEndpoint(_ endpoint: NWEndpoint, timeoutSeconds: Double) async -> Bool {
        ...
        connection.start(queue: DispatchQueue(label: "com.moeghashim.hassoon.macos.bridge-loopback-probe"))
        do {
    ...
    private static func discoverBridgeEndpoint(timeoutSeconds: Double) async -> NWEndpoint? {
        ...
        return await withCheckedContinuation { cont in
            ...
            for domain in HassoonBonjour.bridgeServiceDomains {
                let browser = NWBrowser(
                    for: .bonjour(type: HassoonBonjour.bridgeServiceType, domain: domain),
                    using: params)
                ...
                browser.start(queue: DispatchQueue(label: "com.moeghashim.hassoon.macos.bridge-discovery.\(domain)"))
            }
...
enum MacNodeTokenStore {
    private static let suiteName = "com.moeghashim.hassoon.shared"
