...
enum CommandResolver {
    private static let projectRootDefaultsKey = "hassoon.gatewayProjectRootPath"
    private static let helperName = "hassoon"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager.default.isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/hassoon.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/hassoon")
    ...
    static func hassoonExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectHassoonExecutable(projectRoot: URL? = nil) -> String? {
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/hassoon.js").path
        return FileManager.default.isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnyHassoonInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.hassoonExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func hassoonNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let hassoonPath = self.projectHassoonExecutable(projectRoot: root) {
                return [hassoonPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "hassoon", subcommand] + extraArgs
            }
            if let hassoonPath = self.hassoonExecutable(searchPaths: searchPaths) {
                return [hassoonPath, subcommand] + extraArgs
            }

            let missingEntry = """
            hassoon entrypoint missing (looked for dist/index.js or bin/hassoon.js); run pnpm build.
            """
    ...
    // Existing callers still refer to hassoonCommand; keep it as node alias.
    static func hassoonCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        searchPaths: [String]? = nil) -> [String]
    {
        self.hassoonNodeCommand(
            subcommand: subcommand,
            extraArgs: extraArgs,
            defaults: defaults,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real hassoon CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/hassoon"
        ...
        \(cliSection)
        \(projectSection)
        if command -v hassoon >/dev/null 2>&1; then
          CLI="$(command -v hassoon)"
          hassoon \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/hassoon.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/hassoon.js"
            node "$PRJ/bin/hassoon.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent hassoon"
          pnpm --silent hassoon \(quotedArgs);
        else
          echo "hassoon CLI missing on remote host"; exit 127;
        fi
        """
    ...
    static func connectionSettings(defaults: UserDefaults = .standard) -> RemoteSettings {
        ...
        if let modeRaw {
            ...
            let seen = defaults.bool(forKey: "hassoon.onboardingSeen")
