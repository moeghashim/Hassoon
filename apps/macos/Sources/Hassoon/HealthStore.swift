...
final class HealthStore {
    ...
    private static let logger = Logger(subsystem: "com.moeghashim.hassoon", category: "health")
    ...
    var summaryLine: String {
        ...
        if !snap.web.linked {
            if let tg = snap.telegram, tg.configured {
                ...
                return "\(tgLabel) · Not linked — run hassoon login"
            }
            return "Not linked — run hassoon login"
        }
    ...
    var detailLine: String? {
        if let error = self.lastError, !error.isEmpty {
            ...
            if lower.contains("connection refused") {
                return "The gateway control port (127.0.0.1:18789) isn’t listening — restart Hassoon to bring it back."
            }
