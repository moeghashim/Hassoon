...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: HassoonProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: HassoonProtocol.AnyCodable] = [
            "presence": .init([HassoonProtocol.AnyCodable(entry)]),
        ]
        let payload = HassoonProtocol.AnyCodable(payloadMap)
