...
@Suite(.serialized) struct UtilitiesTests {
    ...
    @Test func gatewayEntrypointPrefersDistOverBin() throws {
        ...
        let bin = tmp.appendingPathComponent("bin/hassoon.js")
    ...
    @Test func logLocatorPicksNewestLogFile() throws {
        ...
        let dir = URL(fileURLWithPath: "/tmp/hassoon", isDirectory: true)
        try? fm.createDirectory(at: dir, withIntermediateDirectories: true)

        let older = dir.appendingPathComponent("hassoon-old-\(UUID().uuidString).log")
        let newer = dir.appendingPathComponent("hassoon-new-\(UUID().uuidString).log")
