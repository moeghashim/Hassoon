...
struct IOSBridgeChatTransport: HassoonChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> HassoonChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(HassoonChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> HassoonChatHistoryPayload {
        ...
        return try JSONDecoder().decode(HassoonChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [HassoonChatAttachmentPayload]) async throws -> HassoonChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [HassoonChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(HassoonChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.bridge.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(HassoonGatewayHealthOK.self, from: res))?.ok ?? true
    }
            ...
            let task = Task {
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? JSONDecoder().decode(HassoonGatewayHealthOK.self, from: data))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let json = evt.payloadJSON, let data = json.data(using: .utf8) else { break }
                        if let payload = try? JSONDecoder().decode(HassoonChatEventPayload.self, from: data) {
                            continuation.yield(.chat(payload))
                        }
                    case "agent":
                        guard let json = evt.payloadJSON, let data = json.data(using: .utf8) else { break }
                        if let payload = try? JSONDecoder().decode(HassoonAgentEventPayload.self, from: data) {
