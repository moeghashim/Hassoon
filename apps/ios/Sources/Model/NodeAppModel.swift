...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = HassoonCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = HassoonCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = "main"
        ...
        let message = HassoonCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = HassoonCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__hassoon__/a2ui/").absoluteString + "?platform=ios"
    }
                        ...
                        onInvoke: { [weak self] req in
                            guard let self else {
                                return BridgeInvokeResponse(
                                    id: req.id,
                                    ok: false,
                                    error: HassoonNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                            }
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for hassoon:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case HassoonCanvasCommand.present.rawValue:
                let params = (try? Self.decodeParams(HassoonCanvasPresentParams.self, from: req.paramsJSON)) ??
                    HassoonCanvasPresentParams()
            ...
            case HassoonCanvasCommand.hide.rawValue:
                return BridgeInvokeResponse(id: req.id, ok: true)

            case HassoonCanvasCommand.navigate.rawValue:
                let params = try Self.decodeParams(HassoonCanvasNavigateParams.self, from: req.paramsJSON)
                self.screen.navigate(to: params.url)
                return BridgeInvokeResponse(id: req.id, ok: true)

            case HassoonCanvasCommand.evalJS.rawValue:
                let params = try Self.decodeParams(HassoonCanvasEvalParams.self, from: req.paramsJSON)
                let result = try await self.screen.eval(javaScript: params.javaScript)
                let payload = try Self.encodePayload(["result": result])
                return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)

            case HassoonCanvasCommand.snapshot.rawValue:
                let params = try? Self.decodeParams(HassoonCanvasSnapshotParams.self, from: req.paramsJSON)
            ...
            case HassoonCanvasA2UICommand.reset.rawValue:
                ...
                (() => {
                  if (!globalThis.hassoonA2UI) return JSON.stringify({ ok: false, error: "missing hassoonA2UI" });
                  return JSON.stringify(globalThis.hassoonA2UI.reset());
                })()
                """)
                return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)

            case HassoonCanvasA2UICommand.push.rawValue, HassoonCanvasA2UICommand.pushJSONL.rawValue:
                let messages: [AnyCodable]
                if command == HassoonCanvasA2UICommand.pushJSONL.rawValue {
                    let params = try Self.decodeParams(HassoonCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                    messages = try HassoonCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
                } else {
                    do {
                        let params = try Self.decodeParams(HassoonCanvasA2UIPushParams.self, from: req.paramsJSON)
                ...
                let messagesJSON = try HassoonCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
                let js = """
                (() => {
                  try {
                    if (!globalThis.hassoonA2UI) return JSON.stringify({ ok: false, error: "missing hassoonA2UI" });
                    const messages = \(messagesJSON);
                    return JSON.stringify(globalThis.hassoonA2UI.applyMessages(messages));
                  } catch (e) {
            ...
            case HassoonCameraCommand.snap.rawValue:
                ...
                let params = (try? Self.decodeParams(HassoonCameraSnapParams.self, from: req.paramsJSON)) ??
                    HassoonCameraSnapParams()
            ...
            case HassoonCameraCommand.clip.rawValue:
                let params = (try? Self.decodeParams(HassoonCameraClipParams.self, from: req.paramsJSON)) ??
                    HassoonCameraClipParams()
            ...
            case HassoonScreenCommand.record.rawValue:
                let params = (try? Self.decodeParams(HassoonScreenRecordParams.self, from: req.paramsJSON)) ??
                    HassoonScreenRecordParams()
            ...
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: HassoonNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: HassoonNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
