...
private final class TestChatTransport: @unchecked Sendable, HassoonChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [HassoonChatHistoryPayload]
    private let sessionsResponses: [HassoonChatSessionsListResponse]

    private let stream: AsyncStream<HassoonChatTransportEvent>
    private let continuation: AsyncStream<HassoonChatTransportEvent>.Continuation

    init(
        historyResponses: [HassoonChatHistoryPayload],
        sessionsResponses: [HassoonChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<HassoonChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> HassoonChatHistoryPayload {
        ...
        return self.historyResponses.last ?? HassoonChatHistoryPayload(
        ...
        attachments _: [HassoonChatAttachmentPayload]) async throws -> HassoonChatSendResponse
    {
        ...
        return HassoonChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> HassoonChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? HassoonChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: HassoonChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = HassoonChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = HassoonChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { HassoonChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = HassoonChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = HassoonChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = HassoonChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { HassoonChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
