...
struct ChatMessageBubble: View {
    let message: HassoonChatMessage
    let style: HassoonChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? HassoonChatTheme.userText : HassoonChatTheme.assistantText
            ...
            if !split.images.isEmpty {
                ...
                { (item: ChatMarkdownSplitter.InlineImage) in
                    if let img = item.image {
                        HassoonPlatformImageFactory.image(img)
                            .resizable()
                            .scaledToFit()
                            .frame(maxHeight: 260)
    ...
    private var inlineAttachments: [HassoonChatMessageContent] {
    ...
    private var toolCalls: [HassoonChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [HassoonChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? HassoonChatTheme.userBubble
        }
        if self.style == .onboarding {
            return HassoonChatTheme.onboardingAssistantBubble
        }
        return HassoonChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return HassoonChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: HassoonChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? HassoonChatTheme.userText : HassoonChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: HassoonChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(HassoonChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(HassoonChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatStreamingAssistantBubble: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ChatMarkdownBody(text: self.text, textColor: HassoonChatTheme.assistantText)
        }
        .padding(12)
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [HassoonChatPendingToolCall]
