...
private let chatUILogger = Logger(subsystem: "com.moeghashim.hassoon", category: "HassoonChatUI")

@MainActor
@Observable
public final class HassoonChatViewModel {
    public private(set) var messages: [HassoonChatMessage] = []
    ...
    public var attachments: [HassoonPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [HassoonChatPendingToolCall] = []
    public private(set) var sessions: [HassoonChatSessionEntry] = []
    private let transport: any HassoonChatTransport
    ...
    private var pendingToolCallsById: [String: HassoonChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any HassoonChatTransport) {
    ...
    public var sessionChoices: [HassoonChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [HassoonChatSessionEntry] = []
        for entry in sorted {
        ...
        let mainKey = "main"
        var result: [HassoonChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: HassoonPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [HassoonChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: HassoonChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [HassoonChatMessage]) -> [HassoonChatMessage] {
        var result: [HassoonChatMessage] = []
    ...
    private static func dedupeKey(for message: HassoonChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [HassoonChatMessageContent] = [
            HassoonChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> HassoonChatAttachmentPayload in
            HassoonChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> HassoonChatSessionEntry {
        HassoonChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: HassoonChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: HassoonChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: HassoonAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = HassoonChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> HassoonPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
