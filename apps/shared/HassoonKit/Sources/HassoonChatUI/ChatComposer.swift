...
@MainActor
struct HassoonChatComposer: View {
    @Bindable var viewModel: HassoonChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(HassoonChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(HassoonChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \HassoonPendingAttachment.id)
                { (att: HassoonPendingAttachment) in
                    HStack(spacing: 6) {
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(HassoonChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(HassoonChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(HassoonChatTheme.subtleCard)
        .clipShape(Capsule())
    }
