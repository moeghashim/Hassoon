...
import com.moeghashim.hassoon.node.NodeForegroundService
import com.moeghashim.hassoon.node.VoiceWakeMode
  ...
  LazyColumn(
      ...
      AnimatedVisibility(visible = voiceWakeMode != VoiceWakeMode.Off) {
        Column(verticalArrangement = Arrangement.spacedBy(6.dp), modifier = Modifier.fillMaxWidth()) {
          ListItem(
            ...
            supportingContent = { Text("Listens only while Hassoon is open.") },
            trailingContent = {
              RadioButton(
    ...
    item {
      Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
        Button(
          onClick = {
            val parsed = com.moeghashim.hassoon.node.WakeWords.parseCommaSeparated(wakeWordsText)
    ...
    item {
      ListItem(
        ...
        supportingContent = { Text("Keeps the screen awake while Hassoon is open.") },
