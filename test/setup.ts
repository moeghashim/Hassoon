...
const originalTestHome = process.env.HASSOON_TEST_HOME;

const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "hassoon-test-home-"));
process.env.HOME = tempHome;
process.env.USERPROFILE = tempHome;
process.env.HASSOON_TEST_HOME = tempHome;
...
process.on("exit", () => {
  ...
  restoreEnv("HASSOON_TEST_HOME", originalTestHome);
  try {
