...
# Output:
# - dist/Hassoon.app
# - dist/Hassoon-<version>.zip
# - dist/Hassoon-<version>.dmg

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"

"$ROOT_DIR/scripts/package-mac-app.sh"

APP="$ROOT_DIR/dist/Hassoon.app"
if [[ ! -d "$APP" ]]; then
...
ZIP="$ROOT_DIR/dist/Hassoon-$VERSION.zip"
DMG="$ROOT_DIR/dist/Hassoon-$VERSION.dmg"
NOTARY_ZIP="$ROOT_DIR/dist/Hassoon-$VERSION.notary.zip"
NOTARIZE="${NOTARIZE:-0}"
