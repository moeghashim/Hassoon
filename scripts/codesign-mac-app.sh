...
APP_BUNDLE="${1:-dist/Hassoon.app}"
IDENTITY="${SIGN_IDENTITY:-}"
TIMESTAMP_MODE="${CODESIGN_TIMESTAMP:-auto}"
ENT_TMP_BASE=$(mktemp -t hassoon-entitlements-base.XXXXXX)
ENT_TMP_APP=$(mktemp -t hassoon-entitlements-app.XXXXXX)
ENT_TMP_APP_BASE=$(mktemp -t hassoon-entitlements-app-base.XXXXXX)
ENT_TMP_BUN=$(mktemp -t hassoon-entitlements-bun.XXXXXX)
...
# Sign main binary
if [ -f "$APP_BUNDLE/Contents/MacOS/Hassoon" ]; then
  echo "Signing main binary"; sign_item "$APP_BUNDLE/Contents/MacOS/Hassoon" "$APP_ENTITLEMENTS"
fi
...
if [ -d "$APP_BUNDLE/Contents/Resources/Relay" ]; then
  ...
  if [ -f "$APP_BUNDLE/Contents/Resources/Relay/hassoon" ]; then
    echo "Signing embedded relay"; sign_item "$APP_BUNDLE/Contents/Resources/Relay/hassoon" "$ENT_TMP_BUN"
  fi
fi
