...
ZIP=${1:?"Usage: $0 Hassoon-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com.moeghashim/hassoon/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Hassoon-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com.moeghashim/hassoon/releases/download/v${VERSION}/"}
