#!/usr/bin/env bash
# Reset Hassoon like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${HASSOON_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Hassoon.app/Contents/MacOS/Hassoon"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Hassoon"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Hassoon"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Hassoon"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/com.moeghashim.hassoon.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/hassoon-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${HASSOON_RESTART_LOG:-/tmp/hassoon-restart.log}"
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/com.moeghashim.hassoon 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Hassoon instances"
kill_all_hassoon
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Hassoon"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Hassoon.app" ]]; then
    APP_BUNDLE="/Applications/Hassoon.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Hassoon.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Hassoon.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Hassoon.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set HASSOON_APP_BUNDLE to your installed Hassoon.app"
}
