...
function resolveBrowserBaseUrl(controlUrl?: string) {
  ...
  if (!resolved.enabled) {
    throw new Error(
      "Browser control is disabled. Set browser.enabled=true in ~/.hassoon/hassoon.json.",
    );
  }
...
function createBrowserTool(): AnyAgentTool {
  return {
    label: "Hassoon Browser",
    name: "hassoon_browser",
    description:
      "Control hassoon's dedicated browser (status/start/stop/tabs/open/snapshot/screenshot/actions). Use snapshot+act for UI automation. Avoid act:wait by default; use only in exceptional cases when no reliable UI state exists.",
...
function createDiscordTool(): AnyAgentTool {
  return {
    label: "Hassoon Discord",
    name: "hassoon_discord",
    description:
...
function createGatewayTool(): AnyAgentTool {
  return {
    label: "Hassoon Gateway",
    name: "hassoon_gateway",
    description:
...
export function createHassoonTools(): AnyAgentTool[] {
  return [
    createBrowserTool(),
    createCanvasTool(),
    createNodesTool(),
