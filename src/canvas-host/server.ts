...
(() => {
  ...
  const hasIOS = () => !!(window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.hassoonCanvasA2UIAction);
  const hasAndroid = () => !!(window.hassoonCanvasA2UIAction && typeof window.hassoonCanvasA2UIAction.postMessage === "function");
  const hasHelper = () => typeof window.hassoonSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("hassoon:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Hassoon node canvas.");
      return;
    }
    const ok = window.hassoonSendUserAction({
      name,
      surfaceId: "main",
      sourceComponentId,
      context: { t: Date.now() },
...
function isDisabledByEnv() {
  if (process.env.HASSOON_SKIP_CANVAS_HOST === "1") return true;
...
export async function createCanvasHostHandler(
  ...
  const rootDir = resolveUserPath(
    opts.rootDir ?? path.join(os.homedir(), "hassoon", "canvas"),
  );
    ...
    try {
      ...
      if (!filePath) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Hassoon Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
