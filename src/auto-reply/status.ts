...
type AgentConfig = NonNullable<HassoonConfig["agent"]>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts always live at: ~/.hassoon/sessions/<SessionId>.jsonl
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const webLine = (() => {
    if (args.webLinked === false) {
      return "Web: not linked — run `hassoon login` to scan the QR.";
    }
