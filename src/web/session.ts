...
export function resolveWebAuthDir() {
  return path.join(os.homedir(), ".hassoon", "credentials");
}
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === DisconnectReason.loggedOut) {
            console.error(
              danger("WhatsApp session logged out. Run: hassoon login"),
            );
          }
        }
...
export async function pickProvider(pref: Provider | "auto"): Promise<Provider> {
  ...
  if (!hasWeb) {
    throw new Error(
      "No WhatsApp Web session found. Run `hassoon login --verbose` to link.",
    );
  }
  return choice;
}
