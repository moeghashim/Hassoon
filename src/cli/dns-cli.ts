...
export function registerDnsCli(program: Command) {
  ...
  dns
    .command("setup")
    .description(
      "Set up CoreDNS to serve hassoon.internal for unicast DNS-SD (Wide-Area Bonjour)",
    )
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      console.log("");
      console.log("Recommended ~/.hassoon/hassoon.json:");
      console.log(
        JSON.stringify(
          {
            bridge: { bind: "tailnet" },
      ...
      console.log(`- Restrict to domain (Split DNS): hassoon.internal`);
      ...
      const serverPath = path.join(confDir, "hassoon.internal.server");
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by hassoon dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        console.log("");
        console.log(
          "Note: enable discovery.wideArea.enabled in ~/.hassoon/hassoon.json on the gateway and restart the gateway so it writes the DNS-SD zone.",
        );
      }
    });
}
