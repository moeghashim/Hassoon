...
export const WIDE_AREA_DISCOVERY_DOMAIN = "hassoon.internal.";
export const WIDE_AREA_ZONE_FILENAME = "hassoon.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*hassoon-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaBridgeZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "hassoon";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "hassoon");
  ...
  records.push(
    `_hassoon-bridge._tcp IN PTR ${instanceLabel}._hassoon-bridge._tcp`,
  );
  records.push(
    `${instanceLabel}._hassoon-bridge._tcp IN SRV 0 0 ${opts.bridgePort} ${hostLabel}`,
  );
  records.push(
    `${instanceLabel}._hassoon-bridge._tcp IN TXT ${txt.map(txtQuote).join(" ")}`,
  );
  ...
  return `; hassoon-content-hash: ${contentHash}\n${contentBody}`;
}
