...
function parseDnsSdBrowse(stdout: string): string[] {
  ...
  for (const raw of stdout.split("\n")) {
    const line = raw.trim();
    if (!line || !line.includes("_hassoon-bridge._tcp")) continue;
    if (!line.includes("Add")) continue;
    const match = line.match(/_hassoon-bridge\._tcp\.?\s+(.+)$/);
    if (match?.[1]) {
...
function parseAvahiBrowse(stdout: string): GatewayBonjourBeacon[] {
  ...
  for (const raw of stdout.split("\n")) {
    ...
    if (!line) continue;
    if (line.startsWith("=") && line.includes("_hassoon-bridge._tcp")) {
      if (current) results.push(current);
      const marker = " _hassoon-bridge._tcp";
...
async function discoverViaAvahi(
  ...
  const browse = await runCommandWithTimeout(
    ["avahi-browse", "-rt", "_hassoon-bridge._tcp"],
    { timeoutMs },
  );
