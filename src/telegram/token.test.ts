...
function withTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "hassoon-telegram-token-"));
}
...
describe("resolveTelegramToken", () => {
  ...
  it("prefers env token over config", () => {
    ...
    const cfg = { telegram: { botToken: "cfg-token" } } as HassoonConfig;
  ...
  it("uses tokenFile when configured", () => {
    ...
    const cfg = { telegram: { tokenFile } } as HassoonConfig;
    const res = resolveTelegramToken(cfg);
