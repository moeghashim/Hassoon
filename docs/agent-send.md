---
summary: "Design notes for a direct `hassoon agent` CLI subcommand without WhatsApp delivery"
read_when:
  - Adding or modifying the agent CLI entrypoint
---
# `hassoon agent` (direct-to-agent invocation)

`hassoon agent` lets you talk to the **embedded** agent runtime directly (no chat send unless you opt in), while reusing the same session store and thinking/verbose persistence as inbound auto-replies.
