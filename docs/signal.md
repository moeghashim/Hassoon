...
## Why
- Signal OSS stack is GPL/AGPL; not compatible with Hassoon MIT if bundled.
- signal-cli is unofficial; must stay up to date (Signal server churn).

## The “number model” (important)
- Hassoon is a **device** connected via `signal-cli`.
- If you run `signal-cli` on **your personal Signal account**, Hassoon will **not** respond to messages sent from that same account (loop protection: ignore sender==account).
...
- For “I text her, she texts me back” you want a **separate Signal account/number for the bot**:
  ...
  - You DM the bot number; Hassoon replies back to you

You can still run Hassoon on your own Signal account if your goal is “respond to other people as me”, but not for self-chat.
...
1) Install `signal-cli` (keep Java installed).
   - If you use the CLI wizard, it can auto-install to `~/.hassoon/tools/signal-cli/...`.
...
4) Run gateway; sanity probe:
   - `hassoon gateway call providers.status --params '{"probe":true}'`
   - Expect `signal.probe.ok=true` and `signal.probe.version`.
5) DM the bot number from your phone; Hassoon replies.
...
- Your personal account can run `signal-cli`, but you can’t self-chat (Signal loop protection; Hassoon ignores sender==account).

If you have a second phone:
- Create/activate the bot number on that phone.
- Run `signal-cli link -n "Hassoon"` on your Mac, scan the QR on the bot phone.
...
5) Translate receive payload into Hassoon surface model.
