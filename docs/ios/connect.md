---
summary: "Runbook: connect/pair the iOS node to a Hassoon Gateway and drive its Canvas"
read_when:
...
- You can run the CLI (`hassoon`) on the gateway machine (or via SSH).

## 1) Start the Gateway (with bridge enabled)

Bridge is enabled by default (disable via `HASSOON_BRIDGE_ENABLED=0`).
...
- Set `bridge.bind: "tailnet"` in `~/.hassoon/hassoon.json` on the gateway host.
...
You should see your gateway advertising `_hassoon-bridge._tcp`.

If browse works, but the iOS node can’t connect, try resolving one instance:

```bash
dns-sd -L "<instance name>" _hassoon-bridge._tcp local.
```
...
1) Set up a DNS-SD zone (example `hassoon.internal.`) on the gateway host and publish `_hassoon-bridge._tcp` records.
2) Configure Tailscale split DNS for `hassoon.internal` pointing at that DNS server.
...
```bash
hassoon nodes pending
```

Approve the request:

```bash
hassoon nodes approve <requestId>
```
...
- Via nodes status (paired + connected):
  ```bash
  hassoon nodes status
  ```
- Via Gateway (paired + connected):
  ```bash
  hassoon gateway call node.list --params "{}"
  ```
- Via Gateway presence (legacy-ish, still useful):
  ```bash
  hassoon gateway call system-presence --params "{}"
  ```
...
- `window.__hassoon.canvas`
- `window.__hassoon.ctx` (2D context)
- `window.__hassoon.setStatus(title, subtitle)`
...
1) Create `~/hassoon/canvas/index.html` on the gateway host.

2) Navigate the node to it (LAN):

```bash
hassoon nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18793/__hassoon__/canvas/"}'
```

Notes:
- The server injects a live-reload client into HTML and reloads on file changes.
- A2UI is hosted on the same canvas host at `http://<gateway-host>:18793/__hassoon__/a2ui/`.
- Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18793/__hassoon__/canvas/`.
...
```bash
hassoon nodes invoke --node "iOS Node" --command canvas.eval --params "$(cat <<'JSON'
{"javaScript":"(() => { const {ctx,setStatus} = window.__hassoon; setStatus('Drawing','…'); ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle='#ff2d55'; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); setStatus(null,null); return 'ok'; })()"}
JSON
)"
```
...
```bash
hassoon nodes invoke --node 192.168.0.88 --command canvas.snapshot --params '{"maxWidth":900}'
```
