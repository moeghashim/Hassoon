...
## Goals
- Build an **iOS app** that acts as a **remote node** for Hassoon:
...
- The Gateway WebSocket server binds to `127.0.0.1:18789` (`src/gateway/server.ts`) with an optional `HASSOON_GATEWAY_TOKEN`.
- The Gateway exposes a Canvas file server (`canvasHost`) on `canvasHost.port` (default `18793`), so nodes can `canvas.navigate` to `http://<lanHost>:18793/__hassoon__/canvas/` and auto-reload on file changes (`docs/configuration.md`).
...
### Pairing
- Bonjour discovery shows a candidate “Hassoon Bridge” on the LAN.
- First connection:
  ...
  4) Approval can happen via:
     - **macOS UI** (Hassoon shows an alert with Approve/Reject/Later, including the node IP), or
...
Pairing decisions must be owned by the Gateway (`hassoon` / Node) so nodes can be approved without the macOS app running.
...
- If the Swift UI is not present: `hassoon` CLI can list pending requests and approve/reject.
...
- `hassoon nodes reject <requestId>`
...
- A2UI (mobile + macOS canvas):
  ...
  - `canvas.a2ui.reset`
  - A2UI is hosted by the Gateway canvas host (`/__hassoon__/a2ui/`) on `canvasHost.port`. Commands fail if the host is unreachable.
...
```bash
hassoon nodes invoke --node ios-node --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__hassoon; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```
...
- `HassoonProtocol` (generated models; platform-neutral)
- `HassoonGatewayClient` (shared WS framing + connect/req/res + seq-gap handling)
- `HassoonKit` (node/canvas command types + deep links + shared utilities)
...
cd apps/ios
xcodegen generate
open Hassoon.xcodeproj
```

## Storage plan (private by default)
### iOS
- Canvas/workspace files (persistent, private):
  - `Application Support/Hassoon/canvas/<sessionKey>/...`
- Snapshots / temp exports (evictable):
  - `Library/Caches/Hassoon/canvas-snapshots/<sessionKey>/...`
- Credentials:
...
- Pairing (source of truth):
  - `~/.hassoon/nodes/paired.json`
  - `~/.hassoon/nodes/pending.json` (or `pending/*.json` for auditability)
