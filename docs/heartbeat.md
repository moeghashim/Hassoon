...
- During heartbeat runs, Hassoon treats `HEARTBEAT_OK` as an ack when it appears at
  the **start or end** of the reply. Hassoon strips the token and discards the
...
normal (non-heartbeat) reply, Hassoon strips the token and logs a verbose
