...
## Goals
- Let you talk to Hassoon via a Telegram bot in DMs and groups.
...
2) Configure Hassoon with `TELEGRAM_BOT_TOKEN` (or `telegram.botToken` in `~/.hassoon/hassoon.json`).
3) Run the gateway; it auto-starts Telegram only when a `telegram` config section exists **and** a bot token is set (unless `telegram.enabled = false`).
   - If you prefer env vars, still add `telegram: { enabled: true }` to `~/.hassoon/hassoon.json` and set `TELEGRAM_BOT_TOKEN`.
...
## Safety & ops
- Treat the bot token as a secret (equivalent to account control); prefer `TELEGRAM_BOT_TOKEN` or a locked-down config file (`chmod 600 ~/.hassoon/hassoon.json`).
