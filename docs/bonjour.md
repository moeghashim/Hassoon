...
Hassoon uses Bonjour (mDNS / DNS-SD) as a **LAN-only convenience** to discover a running Gateway bridge transport. It is best-effort and does **not** replace SSH or Tailnet-based connectivity.
...
2) Publish DNS-SD records for `_hassoon-bridge._tcp` in a dedicated zone (example: `hassoon.internal.`).
3) Configure Tailscale **split DNS** so `hassoon.internal` resolves via that DNS server for clients (including iOS).

Hassoon standardizes on the discovery domain `hassoon.internal.` for this mode. iOS/Android nodes browse both `local.` and `hassoon.internal.` automatically (no per-device knob).

### Gateway config (recommended)

On the gateway host (the machine running the Gateway bridge), add to `~/.hassoon/hassoon.json` (JSON5):

```json5
{
  bridge: { bind: "tailnet" }, // tailnet-only (recommended)
  discovery: { wideArea: { enabled: true } } // enables hassoon.internal DNS-SD publishing
}
```
...
- serve the zone `hassoon.internal.` from the gateway-owned zone file `~/.hassoon/dns/hassoon.internal.db`
...
```bash
dns-sd -B _hassoon-bridge._tcp hassoon.internal.
dig @<TAILNET_IPV4> -p 53 _hassoon-bridge._tcp.hassoon.internal PTR +short
```
...
- Add split DNS so the domain `hassoon.internal` uses that nameserver.

Once clients accept tailnet DNS, iOS nodes can browse `_hassoon-bridge._tcp` in `hassoon.internal.` without multicast.
...
- Set `bridge.bind: "tailnet"` in `~/.hassoon/hassoon.json`.
...
Only the **Node Gateway** (`hassoon` / `hassoon gateway`) advertises Bonjour beacons.
...
- `_hassoon-bridge._tcp` — bridge transport beacon (used by macOS/iOS/Android nodes).
...
- `canvasPort=<port>` (only when the canvas host is enabled + reachable; default `18793`; serves `/__hassoon__/canvas/`)
- `cliPath=<path>` (optional; absolute path to a runnable `hassoon` entrypoint or binary)
...
- Browse instances:
  - `dns-sd -B _hassoon-bridge._tcp local.`
- Resolve one instance (replace `<instance>`):
  - `dns-sd -L "<instance>" _hassoon-bridge._tcp local.`
...
The iOS node app discovers bridges via `NWBrowser` browsing `_hassoon-bridge._tcp`.
...
- UIs should decode for display (iOS uses `BonjourEscapes.decode` in `apps/shared/HassoonKit`).

## Disabling / configuration

- `HASSOON_DISABLE_BONJOUR=1` disables advertising.
- `HASSOON_BRIDGE_ENABLED=0` disables the bridge listener (and therefore the bridge beacon).
- `bridge.bind` / `bridge.port` in `~/.hassoon/hassoon.json` control bridge bind/port (preferred).
- `HASSOON_BRIDGE_HOST` / `HASSOON_BRIDGE_PORT` still work as a back-compat override when `bridge.bind` / `bridge.port` are not set.
- `HASSOON_SSH_PORT` overrides the SSH port advertised in `_hassoon-bridge._tcp`.
- `HASSOON_TAILNET_DNS` publishes a `tailnetDns` hint (MagicDNS) in `_hassoon-bridge._tcp`. If unset, the gateway auto-detects Tailscale and publishes the MagicDNS name when possible.
