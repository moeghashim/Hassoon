---
summary: "Spec for the Hassoon macOS companion menu bar app (gateway + node broker)"
read_when:
  - Implementing macOS app features
  - Changing gateway lifecycle or node bridging on macOS
---
# Hassoon macOS Companion (menu bar + gateway broker)

Author: steipete · Status: draft spec · Date: 2025-12-20

## Purpose
- Single macOS menu-bar app named **Hassoon** that:
  - Shows native notifications for Hassoon/hassoon events.
  ...
  - Installs a single CLI (`hassoon`) by symlinking the bundled binary.

## High-level design
- SwiftPM package in `apps/macos/` (macOS 15+, Swift 6).
- Targets:
  - `HassoonIPC` (shared Codable types + helpers for app‑internal actions).
  - `Hassoon` (LSUIElement MenuBarExtra app; hosts Gateway + node bridge + PeekabooBridgeHost).
- Bundle ID: `com.moeghashim.hassoon`.
- Bundled runtime binaries live under `Contents/Resources/Relay/`:
  - `hassoon` (bun‑compiled relay: CLI + gateway-daemon)
- The app symlinks `hassoon` into `/usr/local/bin` and `/opt/homebrew/bin`.
...
## CLI (`hassoon`)
- The **only** CLI is `hassoon` (TS/bun). There is no `hassoon-mac` helper.
- For mac‑specific actions, the CLI uses `node.invoke`:
  - `hassoon canvas present|navigate|eval|snapshot|a2ui push|a2ui reset`
  - `hassoon nodes run --node <id> -- <command...>`
  - `hassoon nodes notify --node <id> --title ...`
...
Hassoon (the macOS app) registers a URL scheme for triggering local actions from anywhere (browser, Shortcuts, CLI, etc.).
...
```bash
open 'hassoon://agent?message=Hello%20from%20deep%20link'
```
...
- With `key=<value>`, Hassoon runs without prompting (intended for personal automations).
  - The current key is shown in Debug Settings and stored locally in UserDefaults.

Notes:
- In local mode, Hassoon will start the local Gateway if needed before issuing the request.
- In remote mode, Hassoon will use the configured remote tunnel/endpoint.

## Build & dev workflow (native)
- `cd native && swift build` (debug) / `swift build -c release`.
- Run app for dev: `swift run Hassoon` (or Xcode scheme).
