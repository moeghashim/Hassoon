...
5. **Logging**
   - Coordinator emits `.info` logs in subsystem `com.moeghashim.hassoon`, categories `voicewake.overlay` and `voicewake.chime`.
...
- Stream logs while reproducing a sticky overlay:
  ...
  ```bash
  sudo log stream --predicate 'subsystem == "com.moeghashim.hassoon" AND category CONTAINS "voicewake"' --level info --style compact
  ```
