...
## How the probe works
- App runs `hassoon health --json` via `ShellExecutor` every ~60s and on demand. The probe loads creds, attempts a short Baileys connect, and reports status without sending messages.
- Cache the last good snapshot and the last error separately to avoid flicker; show the timestamp of each.

## When in doubt
- You can still use the CLI flow in `docs/health.md` (`hassoon status`, `hassoon status --deep`, `hassoon health --json`) and tail `/tmp/hassoon/hassoon-*.log` for `web-heartbeat` / `web-reconnect`.
