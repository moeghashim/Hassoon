...
---
# Hassoon gateway as a child process of the macOS app
...
## Goal
Run the Node-based Hassoon/hassoon gateway as a direct child of the LSUIElement app (instead of a launchd agent) while keeping all TCC-sensitive work inside the Swift app/broker layer and wiring the existing “Hassoon Active” toggle to start/stop the child.

## When to prefer the child-process mode
- You want gateway lifetime strictly coupled to the menu-bar app (dies when the app quits) and controlled by the “Hassoon Active” toggle without touching launchd.
...
- Add a small `GatewayProcessManager` (Swift) that owns:
  ...
  - `start(config)` called when “Hassoon Active” flips ON:
    - binary: host Node running the bundled gateway under `Hassoon.app/Contents/Resources/Gateway/`
    - args: current hassoon entrypoint and flags
    - cwd/env: point to `~/.hassoon` as today; inject the expanded PATH so Homebrew Node resolves under launchd
    - output: stream stdout/stderr to `/tmp/hassoon-gateway.log` (cap buffer via Subprocess OutputLimits)
...
- Stream child stdout/stderr to `/tmp/hassoon-gateway.log`; surface the last N lines in the Debug tab.
...
- Respect the existing pause semantics: when paused, the broker should return `ok=false, "hassoon paused"`; the gateway should avoid calling privileged routes while paused.
...
- Implement `GatewayProcessManager` with Swift Subprocess to start/stop the gateway on the “Hassoon Active” toggle.
