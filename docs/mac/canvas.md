...
Note: for iOS/Android nodes that should render agent-edited HTML/CSS/JS over the network, prefer the Gateway `canvasHost` (serves `~/hassoon/canvas` over LAN/tailnet with live reload). A2UI is also **hosted by the Gateway** over HTTP. This doc focuses on the macOS in-app canvas panel. See `docs/configuration.md`.

Hassoon can embed an agent-controlled “visual workspace” panel (“Canvas”) inside the macOS app using `WKWebView`, served via a **custom URL scheme** (no loopback HTTP port required).
...
- `hassoon-canvas://<session>/<path>`

Routing model:
- `hassoon-canvas://main/` → `<canvasRoot>/main/index.html` (or `index.htm`)
- `hassoon-canvas://main/yolo` → `<canvasRoot>/main/yolo/index.html` (or `index.htm`)
- `hassoon-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
...
- `~/Library/Application Support/Hassoon/canvas/<session>/…`

This keeps it alongside other app-owned state and avoids mixing with `~/.hassoon/` gateway config.
...
This should be modeled after `WebChatManager`/`WebChatSwiftUIWindowController` but targeting `hassoon-canvas://…` URLs.

Related:
- For “invoke the agent again from UI” flows, prefer the macOS deep link scheme (`hassoon://agent?...`) so *any* UI surface (Canvas, WebChat, native views) can trigger a new agent run. See `docs/hassoon-mac.md`.

## Agent commands (current)

Use the main `hassoon` CLI; it invokes canvas commands via `node.invoke`.

- `hassoon canvas present [--node <id>] [--target <...>] [--x/--y/--width/--height]`
...
- `hassoon canvas hide [--node <id>]`
- `hassoon canvas eval --js <code> [--node <id>]`
- `hassoon canvas snapshot [--node <id>]`
...
```
http://<gateway-host>:18793/__hassoon__/a2ui/
```
...
- `hassoon canvas a2ui push --jsonl <path> [--node <id>]`
- `hassoon canvas a2ui reset [--node <id>]`
...
hassoon canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```
...
- Quick smoke: `hassoon canvas a2ui push --text "Hello from A2UI"` renders a minimal v0.8 view.
...
This is intentionally separate from `hassoon-canvas://…` (which is only for serving local Canvas files into the `WKWebView`).

Suggested patterns:
- HTML: render links/buttons that navigate to `hassoon://agent?message=...`.
- JS: set `window.location.href = 'hassoon://agent?...'` for “run this now” actions.

Implementation note (important):
- In `WKWebView`, intercept `hassoon://…` navigations in `WKNavigationDelegate` and forward them to the app, e.g. by calling `DeepLinkHandler.shared.handle(url:)` and returning `.cancel` for the navigation.

Safety:
- Deep links (`hassoon://agent?...`) are always enabled.
...
- Navigation policy: allow only `hassoon-canvas://…` (and optionally `about:blank`); open `http/https` externally.
...
- Log scheme requests + resolution decisions to OSLog (subsystem `com.moeghashim.hassoon`, category `Canvas`).
