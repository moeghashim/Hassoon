---
summary: "Plan for integrating Peekaboo automation into Hassoon via PeekabooBridge (socket-based TCC broker)"
read_when:
  - Hosting PeekabooBridge in Hassoon.app
  - Integrating Peekaboo as a submodule
  - Changing PeekabooBridge protocol/paths
---
# Peekaboo Bridge in Hassoon (macOS UI automation broker)
...
- Hassoon integrates by **optionally hosting the same bridge** inside **Hassoon.app** (user-toggleable). The primary client is the **`peekaboo` CLI** (installed via npm); Hassoon does not need its own `ui …` CLI surface.
- For **visualizations**, we keep them in **Peekaboo.app** (best UX); Hassoon stays a thin broker host. No visualizer toggle in Hassoon.

Non-goals:
- No auto-launching Peekaboo.app.
- No onboarding deep links from the automation endpoint (Hassoon onboarding already handles permissions).
- No AI provider/agent runtime dependencies in Hassoon (avoid pulling Tachikoma/MCP into the Hassoon app/CLI).

## Big refactor (Dec 2025): XPC → Bridge
Peekaboo’s privileged execution moved from “CLI → XPC helper” to “CLI → socket bridge host”. For Hassoon this is a win:
- It matches the existing “local socket + codesign checks” approach.
- It lets us piggyback on **either** Peekaboo.app’s permissions **or** Hassoon.app’s permissions (whichever is running).
...
- Hassoon: `~/Library/Application Support/hassoon/bridge.sock`

No auto-launch: if a host isn’t reachable, the command fails with a clear error (start Peekaboo.app, Claude.app, or Hassoon.app).
...
Path in Hassoon repo:
- `./Peekaboo` (Swabble-style; keep stable so SwiftPM path deps don’t churn).

What Hassoon should use:
- **Client side**: `PeekabooBridge` (socket client + protocol models).
- **Host side (Hassoon.app)**: `PeekabooBridgeHost` + the minimal Peekaboo services needed to implement operations.

What Hassoon should *not* embed:
...
## IPC / CLI surface
### No `hassoon ui …`
We avoid a parallel “Hassoon UI automation CLI”. Instead:
- `peekaboo` is the user/agent-facing CLI surface for automation and capture.
- Hassoon.app can host PeekabooBridge as a **thin TCC broker** so Peekaboo can piggyback on Hassoon permissions when Peekaboo.app isn’t running.
...
- Hassoon.app converts to global CG coordinates using `NSScreen.screens[screenIndex].frame.origin`.
...
- Hassoon hosts the bridge, but does not render overlays.
- Any “visualizer enabled/disabled” setting is controlled in Peekaboo.app.

## Screenshots (legacy → Peekaboo takeover)
Hassoon should not grow a separate screenshot CLI surface.

Migration plan:
- Use `peekaboo capture …` / `peekaboo see …` (returns a file path, default temp directory).
- Once Hassoon’ legacy screenshot plumbing is replaced, remove it cleanly (no aliases).
...
2. Host `PeekabooBridgeHost` inside Hassoon.app behind a single setting (“Enable Peekaboo Bridge”, default on).
3. Ensure Hassoon hosts the bridge at `~/Library/Application Support/hassoon/bridge.sock` and speaks the PeekabooBridge JSON protocol.
4. Validate with `peekaboo bridge status --verbose` that Peekaboo can select Hassoon as the fallback host (no auto-launch).
