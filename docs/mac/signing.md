...
- sets a stable debug bundle identifier: `com.moeghashim.hassoon.debug`
...
- inject build metadata into Info.plist: `HassoonBuildTimestamp` (UTC) and `HassoonGitCommit` (short hash) so the About pane can show build, git, and debug/release channel.
...
- `HassoonBuildTimestamp`: ISO8601 UTC at package time
- `HassoonGitCommit`: short git hash (or `unknown` if unavailable)
...
TCC permissions are tied to the bundle identifier *and* code signature. Unsigned debug builds with changing UUIDs were causing macOS to forget grants after each rebuild. Signing the binaries (ad‑hoc by default) and keeping a fixed bundle id/path (`dist/Hassoon.app`) preserves the grants between builds, matching the VibeTunnel approach.
