...
- By default: the Gateway exposes a Canvas host on `canvasHost.port` (default `18793`), serving `~/hassoon/canvas` at `/__hassoon__/canvas/` with live-reload; disable via `canvasHost.enabled=false` or `HASSOON_SKIP_CANVAS_HOST=1`.
...
- **Clients (mac app / CLI / web admin)**  
  ...
  - On macOS, the app can also be invoked via deep links (`hassoon://agent?...`) which translate into the same Gateway `agent` request path (see `docs/hassoon-mac.md`).
- **Agent process (Pi)**  
...
- If `HASSOON_GATEWAY_TOKEN` (or `--token`) is set, `connect.params.auth.token` must match; otherwise the socket closes with policy violation.
...
## Operations snapshot
- Start: `hassoon gateway` (foreground, logs to stdout).  
