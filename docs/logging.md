...
Hassoon uses a file logger backed by `tslog` (`src/logging.ts`).

- Default rolling log file is under `/tmp/hassoon/` (one file per day): `hassoon-YYYY-MM-DD.log`
- The log file path and level can be configured via `~/.hassoon/hassoon.json`:
  - `logging.file`
  - `logging.level`
...
`hassoon gateway` supports a per-gateway style switch:
...
hassoon gateway --verbose --ws-log compact

# show all WS traffic (full meta)
hassoon gateway --verbose --ws-log full
```

## Console formatting (subsystem logging)

Hassoon formats console logs via a small wrapper on top of the existing stack:
