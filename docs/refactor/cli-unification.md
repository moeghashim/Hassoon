---
summary: "Refactor: unify on the hassoon CLI + gateway-first control; retire hassoon-mac"
read_when:
...
# CLI unification (hassoon-only)

Status: active refactor · Date: 2025-12-20

## Goals
- **Single CLI**: use `hassoon` for all automation (local + remote). Retire `hassoon-mac`.
...
## Non-goals
- Keep legacy `hassoon-mac` compatibility.
...
3) **Mac app installs/symlinks `hassoon`**
   - Bundle a standalone `hassoon` binary in the app (bun-compiled).
   - Install/symlink that binary to `/usr/local/bin/hassoon` and `/opt/homebrew/bin/hassoon`.
   - No `hassoon-mac` helper remains.
...
- `hassoon nodes invoke --command canvas.*` remains valid.
...
- When config is missing or explicitly remote, `hassoon gateway` should refuse to auto-start unless forced.

## Implementation checklist
- Add bun-compiled `hassoon` binary to macOS app bundle; update codesign + install flows.
- Remove `HassoonCLI` target and control socket server.
- Add node command(s) for `system.run` and `system.notify` on macOS.
- Add permission map to node hello/pairing + gateway responses.
- Update TS CLI + docs to use `hassoon` only.
