...
## Phase 0 — Foundations
- **Naming**: CLI subcommand `hassoon gateway`; internal namespace `Gateway`.
...
- **Schema tooling**:
  ...
  - `pnpm protocol:gen:swift`: generates Swift `Codable` models (`apps/macos/Sources/HassoonProtocol/GatewayModels.swift`). ✅
...
- Payload types:
  ...
  - `HealthSnapshot` (match existing `hassoon health --json` fields)
...
- Add `hassoon gateway` command in CLI program:
...
- Retire `hassoon rpc` as default path; keep only if explicitly requested (documented as legacy).
...
- Ship launchd/systemd templates for `hassoon gateway`.
