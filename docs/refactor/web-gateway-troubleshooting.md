...
- **Stream Errored / Conflict / status 409–515:** WhatsApp closed the socket because another session is active or creds went stale. Run `hassoon logout`, then `hassoon login`, then restart the Gateway.
- **Logged out:** Console prints “session logged out”; re-link with `hassoon login`.
...
## Helpful commands
- Start the Gateway: `hassoon gateway --verbose`
- Logout (clear creds): `hassoon logout`
- Relink (show QR): `hassoon login --verbose`
- Tail logs (default): `tail -f /tmp/hassoon/hassoon-*.log`
...
## If it keeps failing
1) `hassoon logout` → `hassoon login` (fresh QR link).
