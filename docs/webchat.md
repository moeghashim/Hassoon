...
- Gateway WS is configured via the app’s gateway endpoint settings (`GatewayEndpointStore`) or `hassoon gateway --port` when running locally.
...
## Dev notes
- macOS glue: `apps/macos/Sources/Hassoon/WebChatSwiftUI.swift` + `apps/macos/Sources/Hassoon/WebChatManager.swift`.
- Remote tunnel helper: `apps/macos/Sources/Hassoon/RemotePortTunnel.swift`.
