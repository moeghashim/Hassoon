...
Goal: let Hassoon sit in WhatsApp groups, wake up only when pinged, and keep that thread separate from the personal DM session.
...
## Config for Hassoon UK (+447700900123)
Add a `groupChat` block to `~/.hassoon/hassoon.json` so display-name pings work even when WhatsApp strips the visual `@` in the text body:
...
Notes:
- The regexes are case-insensitive; they cover `@hassoon`, `@hassoon uk`, `hassoonbot`, and the raw number with or without `+`/spaces.
...
## How to use
1) Add Hassoon UK (`+447700900123`) to the group.
2) Say `@hassoon …` (or `@hassoon uk`, `@hassoonbot`, or include the number). Anyone in the group can trigger it.
...
- Manual smoke:
  - Send an `@hassoon` ping in the group and confirm a reply that references the sender name.
...
- Session store entries will appear as `whatsapp:group:<jid>` in the session store (`~/.hassoon/sessions/sessions.json` by default); a missing entry just means the group hasn’t triggered a run yet.
